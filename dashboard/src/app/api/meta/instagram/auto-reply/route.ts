...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      firstTurnInstruction: settings.AI_FIRST_TURN_INSTRUCTION || `CONTEXT: First message from this user. Greet them casually in lowercase like "heyy, kya haal hai" or "hi, bolo". Do not ask multiple questions.`,
      ongoingTurnInstruction: settings.AI_ONGOING_TURN_INSTRUCTION || `CONTEXT: Ongoing conversation. Continue naturally from the chat history in casual lowercase text.`,
      systemRules: settings.AI_SYSTEM_RULES || `CRITICAL INSTRUCTIONS (NEVER BREAK THESE):\n- LANGUAGE MATCHING (CRITICAL): Detect the language of the user's message (Hindi/Hinglish, English, etc.) and ALWAYS reply in the VERY SAME LANGUAGE.\n- WHEN USER ASKS FOR PHOTOS, VIDEOS, PICS, EXCLUSIVE CONTENT, OR TO SEE YOU:\n  * Be lovely, sweet, and flirty in your response.\n  * Share the Premium Content link (https://smritishans.mywebsite.social/) for exclusive/spicy content.\n  * ALSO share the Free Daily Gallery link (https://smritishah.vercel.app/) as a free trial to check out daily free uploads first!\n- WHEN USER SAYS THEY CAN'T PAY OR HAVE NO MONEY FOR PREMIUM:\n  * Be super sweet and understanding. Tell them no worries at all and invite them to check out your free daily gallery at https://smritishah.vercel.app/.\n- FOR NORMAL CHATS & GREETINGS: Just chat back naturally without link spam.\n- TEXTING STYLE: Write in casual lowercase text.\n- NO EMOJIS: Do not use emojis at all (0 emojis).\n- NO EXCESSIVE PUNCTUATION: Do not use exclamation marks (!), multiple question marks (??), or formal capital letters.`,
      subscriptionLink: settings.AI_SUBSCRIPTION_LINK || `https://smritishans.mywebsite.social/`,
      freeGalleryLink: settings.AI_FREE_GALLERY_LINK || `https://smritishah.vercel.app/`,
